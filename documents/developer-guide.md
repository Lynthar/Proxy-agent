...
install.sh 按下列顺序 `source` 模块（install.sh 第 346-351 行的 for 循环）：
...
所有链式状态文件通过 `writeChainInfoAtomic`（install.sh L4990）写入：JSON 语法校验 → mktemp → rename。失败保留旧文件不变。外部节点 add/remove 用 `addExternalNodeToFile` / `removeExternalNodeFromFile`，同样走原子写入 + jq 返回码检查。
